...
namespace
{
    ...
    bool PaintCurrentFrame(MovieSlot& slot, bool forceLog)
    {
        ...
        // This goes through the previously installed geometry/IV50 hook chain,
        // but does not re-enter this hook because g_next points at the next IAT
        // target captured before this layer was installed.
        ...
        if (!MapWindowPoints(slot.mciWindow, paintWindow, &targetPoint, 1))
        {
        ...
        {
            ShimLog(
                "moviepresent: device=%u MCI_UPDATE=success target=%p visibleAncestor=%p dest=%ld,%ld %dx%d mapped=%ld,%ld blit=%d",
                static_cast<unsigned int>(slot.deviceId),
                slot.mciWindow,
                paintWindow,
                slot.destination.left,
    ...
    void StartTimer(MovieSlot& slot)
    {
        ...
        // Deliberately defer the first MCI_UPDATE until after Battlezone's
        // mciSendCommand call stack has unwound. The prior experiment crashed
        // because it manipulated the MCI target window synchronously here.
